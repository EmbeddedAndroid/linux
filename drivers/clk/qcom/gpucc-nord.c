 ...
 *
 * GPU clock controller for the Nord GPU_0 block.
 *
 * The NORD GPU_CC sits at the same base as the SM8650 GPU_CC and drives an
 * Adreno-7xx GPU + GMU, but the internal register layout (RCG/branch/GDSC
 * offsets, both alpha PLLs LUCID_OLE) differs, so it needs its own offset
 * tables. The clock tree and parent topology mirror SM8650.
 */
...
#include <dt-bindings/reset/qcom,nord-gpucc.h>
...
static const struct qcom_reset_map gpu_cc_nord_resets[] = {
	[GPUCC_GPU_CC_ACD_BCR] = { 0x92f8 },
	[GPUCC_GPU_CC_CB_BCR] = { 0x9340 },
	[GPUCC_GPU_CC_CX_BCR] = { 0x90e4 },
	[GPUCC_GPU_CC_FAST_HUB_BCR] = { 0x9384 },
	[GPUCC_GPU_CC_GFX3D_AON_BCR] = { 0x91ac },
	[GPUCC_GPU_CC_GX_BCR] = { 0x9058 },
	[GPUCC_GPU_CC_RBCPR_BCR] = { 0x91f4 },
...
static int gpu_cc_nord_probe(struct platform_device *pdev)
{
	...
	regmap = qcom_cc_map(pdev, &gpu_cc_nord_desc);
	if (IS_ERR(regmap))
	...
	return qcom_cc_really_probe(&pdev->dev, &gpu_cc_nord_desc, regmap);
}
...
MODULE_DESCRIPTION("Qualcomm Nord GPU clock controller");
MODULE_LICENSE("GPL");
