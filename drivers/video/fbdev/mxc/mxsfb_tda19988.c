...
static void tda19988_reset(struct tda19988_data *data)
{
	...
	/* Setup RGB Muxing for MX7 */
	reg_write(data,REG_VIP_CNTRL_0,0x23);
	reg_write(data,REG_VIP_CNTRL_1,0x45);
	reg_write(data,REG_VIP_CNTRL_2,0x01);
}
		...
		const struct i2c_device_id *id)
{
    ...
    // was 0x70
    data->hdmi = i2c_new_dummy(client->adapter, 0x73);	
	if (!data->hdmi) {
    ...
    // dump out the PXP memory
    int offset=0;
    int value=0;
    int virt_base=0;
    virt_base = ioremap(0x30700000, PAGE_SIZE);
