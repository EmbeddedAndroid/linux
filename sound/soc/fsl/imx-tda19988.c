...
/* Audio portion */
static struct snd_soc_codec_driver mxs_hdmi_codec;

struct imx_tda19988_data {
...
static struct snd_soc_dai_driver mxs_hdmi_codec_dai = {
	.name = "imx7-tda19988",
	.playback = {
		.channels_min = 2,
		.channels_max = 2,
...
static const struct of_device_id imx_tda19988x_dt_ids[] = {
        { .compatible = "mxs,imx-audio-tda19988x", },
        { }
};
MODULE_DEVICE_TABLE(of, imx_tda19988x_dt_ids);

static int imx_tda19988_startup(struct snd_pcm_substream *substream)
{
    ...
    struct imx_tda19988_data *data = snd_soc_card_get_drvdata(card);
...
static int imx_tda19988_hw_free(struct snd_pcm_substream *substream)
{
    struct snd_soc_pcm_runtime *rtd = substream->private_data;
    struct snd_soc_card *card = rtd->card;
    struct imx_tda19988_data *data = snd_soc_card_get_drvdata(card);
...
static void imx_tda19988_shutdown(struct snd_pcm_substream *substream)
{
    struct snd_soc_pcm_runtime *rtd = substream->private_data;
    struct snd_soc_card *card = rtd->card;
    struct imx_tda19988_data *data = snd_soc_card_get_drvdata(card);
...
static struct snd_soc_ops imx_tda19988x_ops = {
    .startup = imx_tda19988_startup,
    .shutdown  = imx_tda19988_shutdown,
    .hw_params = imx_tda19988_hw_params,
    .hw_free = imx_tda19988_hw_free,
};

static int mxs_hdmi_codec_probe(struct platform_device *pdev)
{
    struct device_node *cpu_np;
	struct device_node *tda19988_np;
    struct platform_device *cpu_pdev;
    struct imx_tda19988_data *data;
	int ret;
   ...
   cpu_np = of_parse_phandle(pdev->dev.of_node, "cpu-dai", 0);
    ...
    data->dai.ops = &imx_tda19988x_ops;
...
#if 0
	return snd_soc_register_codec(&pdev->dev, &mxs_hdmi_codec,
			&mxs_hdmi_codec_dai, 1);
#endif
}

static int mxs_hdmi_codec_remove(struct platform_device *pdev)
{
	printk(KERN_ERR "In %s\n", __FUNCTION__);
...
static struct platform_driver mxs_hdmi_codec_driver = {
	.driver		= {
		.name	= "imx-tda19988x",
		.owner	= THIS_MODULE,
		.pm 	= &snd_soc_pm_ops,
		.of_match_table = imx_tda19988x_dt_ids, 
	},

	.probe		= mxs_hdmi_codec_probe,
	.remove		= mxs_hdmi_codec_remove,
};

module_platform_driver(mxs_hdmi_codec_driver);
